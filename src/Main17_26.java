...
public class Main17_26 {
    /**
     * 我所写的方法超过时间了。期待有更好的方法。
     *
     ...
     *
     * 输入为一个二维数组 docs，docs[i] 表示 id 为 i 的文档。返回一个数组，其中每个元素是一个字符串，代表每对相似度大于 0 的文档，其格式为 {id1},{id2}: {similarity}，
    ...
    public static List<String> computeSimilarities(int[][] docs) {
        ...
        for(int i = 0; i < docs.length; i++){
            for(int j = i + 1; j < docs.length; j++){
                if(computeSimilarities(docs[i], docs[j]) != 0){
                    list.add(String.format("%s,%s: %6.4f", i, j, computeSimilarities(docs[i], docs[j])));
                }
            }
        }
        return list;
    ...
    public static double computeSimilarities(int[] array1, int[] array2){
        int n = 0;  //用来记录重复数字的个数
        ...
        return (double) n / (array1.length + array2.length - n);
    }

    public static void main(String[] args) {
        int[][] docs = {{14, 15, 100, 9, 3}, {32, 1, 9, 3, 5}, {15, 29, 2, 6, 8, 7}, {7, 10}};
        List<String> anss = computeSimilarities(docs);
        for (String ans:anss) {
